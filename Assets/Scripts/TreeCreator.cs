            ...
            for (int y = 0; y < dimensions.y; y++)
            {
                for (int x = 0; x < dimensions.x; x++)
                {
                    ...
                    if (allCubes[x,y,z].GetComponent<Renderer>().sharedMaterial.ToString().Contains("trunk"))
                        blockDetails += "(new Vector3Int(" + (x - halfX) + "," + y + "," + (z - halfZ) + "), BlockStaticData.BlockType.Wood),\n";
                    else
                        blockDetails += "(new Vector3Int(" + (x - halfX) + "," + y + "," + (z - halfZ) + "), BlockStaticData.BlockType.Leaves),\n";
